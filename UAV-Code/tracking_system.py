...
def controllingUAV(coord, w, h, keyboard, tresholdPix = 0):
    y = (coord[0] + coord[2]) / 2
    x = (coord[1] + coord[3]) / 2
    if x < (w / 2) + tresholdPix:
        left(keyboard)
    elif (w / 2) - tresholdPix < x:
        right(keyboard)
    
    if y < (h / 2) + tresholdPix:
        up(keyboard)
    elif (h / 2) - tresholdPix < y:
        down(keyboard)
    print(x, y, w, h)

x, y, w, h = set_pos()
x_scale, y_scale = w - x, h - y
keyboard = Controller()
vehicleChoice = str(input("Which vehicle type do you want to detect and track?\n"))
